...
Some of the variation seen in seasonal data may be due to calendar effects, such as the number of days in a month, weekend effects, etc.
Removing these can make the data simpler, and simpler patterns usually lead to more accurate forecasts.
Consider this adjustment whenever you are dealing with monthly totals.

The following is a time plot of monthly milk production on a farm between 1962 and 1975[^3].

[^3]: Data is from the `fma` R package.
...
Any data that is affected by population changes can be adjusted to give data per person.
For instance, suppose we are tracking the number of hospital beds in a country.
It is better to track the number of beds per 1000 residents instead of the total number of beds.
...
Similarly, when tracking disease counts in a country, it is important to compute the number of cases per 1000 people living there, not just the total number of cases there. 
Let's consider the number of Covid-19-related hospitalisations[^1] in 2021 in USA and South Africa.
...
After the adjustment, we observe
similar peaks/troughs and magnitudes, although the peak in USA comes a little after the one in South Africa. 
...
We have already seen it plotted in @fig-intro-cpi, and display it again in the following table. [^5]

[^5]: Separate CPIs can also be calculated for different sectors of the economy. These will calculate aggregate prices for goods and services belonging only to that sector.
...
they would have to pay $1.156 to buy the the same type of items in November 2023.
...
#| warning: false
#| fig-align: center
#| fig-cap: Adjusted and unadjusted prices for Thai rice
...
cpi_rice %>% filter(as.Date(Month) >= ymd("20190101")) |> 
  ...
  labs(x="Month", y="Dollars", 
       title="Price of 5kg of Thai Premium Rice")
```

The unadjusted price (in green) seems to be relatively stable since Jan 2019. 
However, inflation has been causing the value of the Singapore dollar to drop, which means that rice is actually "cheaper" today than it was 4 years ago.

## Composing transformations

One may compose multiple transformations together to obtain a new transformation of the original time series.
We have already seen an example of this in @sec-transformations-log where we composed differing and the log transform to obtain log returns.
