...
$$
\frac{1}{m}\sum_{j=0}^m \left(x_{k + jp} - T_{k + jp} \right) = \underbrace{\frac{1}{m}\sum_{j=0}^m S_{k + jp}}_{= S_k} + \underbrace{\frac{1}{m}\sum_{j=0}^m R_{k + jp}}_{\to 0}
$$
as $m \to \infty$. {#eq-decomposition-seasonality-estimation}
...
#| message: FALSE
#| warning: FALSE
#| fig-cap: Energy demand in Victoria, Australia in January and February 2013, measured half-hourly, and with a 24-hour moving average (red).
