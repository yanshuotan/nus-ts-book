...
$$ {#eq-summary-acvf}
while for $k = -1,-2,\ldots,-n+1$, we define $\hat\gamma_x(k) := \gamma_x(-k)$.
Here, $\bar x := \frac{1}{n}\sum_{t=1}^n x_t$ is the sample mean of the time series.

Note that even though there only $n-k$ terms in the sum in @eq-summary-acvf, we still divide the sum by $n$.
...
Let us apply this to the `globtemp` dataset measuring global temperature deviations (see also @fig-intro-globtemp.)
...
$$
for $k=0,1,\ldots,n-1$.
Note that this is no longer an even function, nor do we necessarily have $\hat\gamma_{xy}(k) = \hat\gamma_{yx}(k)$.
...
Hence, we can interpret this as saying that the lynx population size is most strongly correlated with the hare population size from the previous year, which agrees with existing theories of [predator-prey dynamics](https://en.wikipedia.org/wiki/Lotka%E2%80%93Volterra_equations).
...
We refer the reader to Chapters 4.4 and 4.5. in @hyndman2018forecasting for more details on these statistics as well as how they may be used.[^1]
...
[^1]: Some of these summary statistics, such as tests for stationarity, will be discussed in part 2 of this book.