...
$\delta$ i called the drift of the walk, and if $\delta = 0$, $(X_t)$ is called a *simple random walk*.
...
This can be done using smoothing techniques, or, if the functional form is known in advanced, via simple linear regression onto a convenient function basis.
...
$$
\bar X_n \sim N\left(\mu, \sum_{|h| < n} \left(1 - \frac{|h|}{n}\right)\gamma_X(h)\right).
$$
...
& = \frac{1}{n^2}\sum_{h=-n}^n \left(1 - \frac{|h|}{n} \right)\gamma_X(h).
\end{split}
$$
...
Typically, the model fit in Step 2 is an AutoRegressive Moving Average (ARMA) model, which we will learn about in @sec-arma.
...
Recall our interpretaion of differencing as discrete differentiation, this phenomenon is analogous to how small changes in derivatives $g'$, $g''$, etc. can lead to large changes in the behavior of the original function $g$.
...
Alternatively, if we let $q_h$ denote the quantile function of $\chi^2_h$, we obtain a p-value via $q^{-1}_h(Q_{BP})$.
...
Note also that if the time series to be tested, $(X_t)$, comprises the residuals of a fitted model with $r$ parameters, then $Q_{LB}$ and $Q_{BP}$ actually converge to $\chi^2_{h-p}$, which tends to have smaller values.
...
Recall our use of classical decomposition to model tourist arrivals to Australia from Japan (@sec-decomposition-examples).
...
#| warning: FALSE
#| message: FALSE
#| fig-cap: Remainder component of a classical decomposition of tourist arrivals to Australia from Japan.
...
#| warning: FALSE
#| message: FALSE
#| fig-cap: ACF of remainder component of a classical decomposition of tourist arrivals to Australia from Japan.
