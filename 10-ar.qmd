...
**Approach 2.** 
Multiply both sides of @eq-ar-arp by $X_{t-h}$ and compute expectations.
...
$$
N(\mu_{n+1} + \Sigma_{1:n,n+1}^T\Sigma_{1:n,1:n}^{-1}(x_{1:n} - \mu_{1:n}), \Sigma_{n+1,n+1}^{-1} - \Sigma_{1:n,n+1}^T\Sigma_{1:n,1:n}^{-1}\Sigma_{1:n,n+1}),
$$
where $\Sigma_{1:n,1:n}$ is the top left $n \times n$ submatrix of $\Sigma$, $\Sigma_{n+1,n+1}$ its bottom right entry, and $\Sigma_{1:n,n+1}$ its bottom row without the bottom right entry，i.e.
$$
\Sigma = \left[\begin{matrix}     \Sigma_{1:n,1:n} & \Sigma_{n+1,1:n}  \\  \Sigma_{n+1,1:n} & \Sigma_{n+1,n+1}
\end{matrix}\right].
$$
...
$$
N(\gamma_{h:n+h-1}^T\Gamma_{n}^{-1}x_{n:1}, \gamma_X(0)^{-1} - \gamma_{h:n+h-1}^T\Gamma_{n}^{-1}\gamma_{h:n+h-1}),
...
$$
\hat x_{n+h|n} = \gamma_{h:n+h}^T\Gamma_{n}^{-1}x_{1:n}
$$ {#eq-ar-forecast-mean}
as well as 95% prediction intervals $(\hat x_{n+h|n} - 1.96v_{n+h}^{1/2}, \hat x_{n+h|n} + 1.96v_{n+h}^{1/2})$ for $v_{n+h} = \gamma(0)_X^{-1} - \gamma_{1:n}^T\Gamma_{n}^{-1}\gamma_{1:n}$.

### Best linear predictors

The point forecast @eq-ar-forecast-mean is the mean of the conditional distribution and can be though of as a function of the random variables, i.e.
$$
...
This approach is sometimes called *direct multi-step forecasting*, to contrast it with another approach called *recurisve multi-step forecasting*.
...
In addition to this, we introduce two other methods which are commonplace in statistics: the method of maximum likelihood, and the method of least squares.
...
& = \frac{n}{2}\log(2\pi\sigma^2) + \sum_{t=1}^p \log (\nu_t^2/\sigma^2) + \frac{S(\mu, \phi_{1:p})}{2\sigma^2}
\end{split}
$$ {#eq-ar-estimation-full-likelihood}
where 
$$
\begin{split}
S(\mu, \phi_{1:n}) & = \sum_{t=1}^p (x_t - \mu - \phi_{1:t-1}^T(x_{t-1:1} -\mu))^2\cdot\frac{\sigma^2}{\nu_t^2} \\
...
$$
\begin{split}
S_c(\mu, \phi_{1:n}) & = \sum_{t=p+1}^n (x_t - \mu - \phi_{1:p}^T(x_{t-1:t-p} -\mu))^2.
\end{split} 
