...
#| label: fig-arma-sunspots-sin-model-fitted
...
#| label: fig-arma-sunspots-sin-model-residuals
sunspots_fit |>
    gg_tsresiduals()
```
...
- The sample trajectory seems much "noisier" than the observed time series.
...
### Periodic behavior from ODEs {#sec-arma-motivation-odes}

Yule's starting observation was the fact that the mean function of @eq-arma-sunspot-sinusoid,
$$
x(t) = \mu + \alpha_1 \cos(\omega t) + \alpha_2 \sin(\omega t),
$$ {#eq-arma-sunspots-mean}
is a solution to the ordinary differential equation (ODE)
$$
x''(t) = - \omega^2 \left(x(t) - \mu\right).
$$ {#eq-arma-sunspots-ode}
Indeed, ODE theory tells us that all solutions to this ODE are of the form @eq-arma-sunspots-mean for some choice of $\alpha_1$ and $\alpha_2$ determined by initial conditions.

An ODE tells us how a physical system evolves deterministically based on its inherent properties.
For instance, @eq-arma-sunspots-ode is also the ODE governing the small angle oscillation of a pendulum resulting from gravity, which means that the pendulum's angular position is given by @eq-arma-sunspots-mean.
...
In order to accommodate this, we can add a noise term into @eq-arma-sunspots-ode to get a *stochastic differential equation*:
$$
d^2X_t = - \omega^2 \left(X_t - \mu\right) + dW_t.
$$
Since time series deals with discrete time points, however, we will first re-interpret @eq-arma-sunspots-ode in a discrete setting.
...
**Proposition 10.1.**
The time series @eq-arma-sunspots-mean is the unique general solution to the difference equation:
$$
(I-B)^2 x_t = 2(\cos \omega - 1)(x_{t+1} - \mu).
$$ {#eq-arma-sunspots-difference-equation}

Here, $B$ is the backshift operator defined in @sec-transformations.
Observe the similarity between @eq-arma-sunspots-ode and @eq-arma-sunspots-difference-equation, especially bearing in mind that the difference operator $I-B$ can be thought of as discrete differentiation.

### An autoregressive model

Following the argument in @sec-arma-motivation-odes, @yule1927method proposed the model:
$$
(I-B)^2 X_t = 2(\cos \omega - 1)(X_{t-1} - \mu) + W_t,
$$ {#eq-arma-sunspots-ar-model}
...
Note that @eq-arma-sunspots-ar-model can be rewritten as
$$
...
#| label: fig-arma-sunspots-ar-model-fitted
...
#| label: fig-arma-sunspots-ar-model-residuals
#| warning: FALSE
sunspots_fit |>
    gg_tsresiduals()
```
...
Furthermore, the fitted values are much closer to the observed time series, and a sample trajectory from the model now seems qualitatively similar to the observed time series.
...
Meanwhile, $X_t$ reflects the our observation of nature at time $t$.
...
Note that a simple rearrangement of @eq-arma-arp shows that it is equivalent to
$$
...
The *autoregressive polynomial* of the model @eq-arma-arp is
$$
\phi(t) = 1 - \phi_1 t - \phi_2 t^2 - \cdots - \phi_p t^p.
$$
Using this notation, we can rewrite @eq-arma-arp as
$$
\phi(B)X_t = W_t,
$$
...
It is important to understand that @eq-arma-arp and its variants are *equations* and that the model is defined as a *solution* to equation.
A priori, such a solution may not exist at all, or there may be multiple solutions.

<!-- As mentioned earlier, there is a close connection between @eq-arma-arp and linear ODEs (see @eq-arma-sunspots-ode).
...
### Causal solutions {#sec-arma-ar1-solutions}

**Proposition 10.1** (Existence and uniqueness of solutions). 
Suppose $|\phi| < 1$, then there is a unique stationary solution to @eq-arma-ar1.
...
$$ 
the expression converges absolutely, so $X_t$ is well-defined.
Indeed this is a causal linear process, and is in particular stationary.
It is also easy to see that it satisfies @eq-arma-ar1:
$$
\begin{split}
...
We may then use @eq-arma-ar1 to write
$$
\begin{split}
Y_t & = \phi Y_{t-1} + W_t \\
...
$$
Y_t - Z_t = \sum_{l+1}^\infty \phi^j W_{t-j} + \phi^l Y_{t-l}.
$$
...
\end{split}
$$
Since the left hand side is independent of $l$, we get $Y_t =_d Z_t$ as we wanted. $\Box$

This theorem allows us to speak of *the* solution to @eq-arma-ar1, and we sometimes informally refer to the equation as the model.
As such, we also refer to @eq-arma-ar1 for $|\phi| < 1$ as a causal $\textnormal{AR}(1)$ model.

### Moments {#sec-arma-ar1-moments}

**Proposition 10.2** (Mean, ACVF, ACF).
Let $(X_t)$ be the solution to @eq-arma-ar1.
...
### Forecasts {#sec-arma-ar1-forecasts}

The recurrence form of @eq-arma-ar1 makes forecasting extremely straightforward.
...
$$ {#eq-arma-ar1-forecast-distribution}
...
We simply recursively apply @eq-arma-ar1 to get
$$
X_{n+h} = W_{n+h} + \phi W_{n+h-1} + \cdots + \phi^{h-1}W_{n+1} + \phi^hX_n.
$$
Since $W_{n+1},\ldots,W_{n+h}$ are independent of $X_1,X_2,\ldots,X_n$, this means that @eq-arma-ar1-forecast-distribution is simply $N\left(\phi^h x_n, \frac{(1-\phi^{2h})\sigma^{2}}{1-\phi^2} \right)$.
...
If we assume that the white noise in @eq-arma-ar1 is Gaussian, then the resulting $\textnormal{AR}(1)$ process is also a Gaussian process, which means that its distribution is fully determined by its mean and ACVF functions.
...
#| label: fig-arma-ar1-backcasting
set.seed(5209)
n <- 80
h <- 20
wn <- rnorm(n - 1)
...
First, Proposition 10.2 tells us that $\phi = \rho_X(1)$, so we may use the estimator $\hat\phi \coloneqq \hat\rho_X(1)$. 
Substituting this into [Theorem 9.2](09-stationary.qmd#sec-stationary-moments) then gives
$$
\sqrt n (\hat\phi - \phi) \to N(0,1-\phi^2).
$$
...
If we start with a *fixed* initial value $X_0 = c$, we can iterate @eq-arma-ar1 to get
$$
...
When $\phi = 1$, then @eq-arma-ar1 is the defining equation for a random walk, which we already know is non-stationary.
It is therefore somewhat unintuitive that there exist unique stationary solutions to @eq-arma-ar1 when $|\phi| > 1$.

**Stationary solution.**
Define the expression
$$
X_t \coloneqq -\sum_{j=1}^\infty \phi^{-j}W_{t+j}.
$$ {#eq-arma-ar1-noncausal}
Using the same arguments as in the proof of [Proposition 10.1](10-arma.qmd#sec-arma-ar1-solutions), one can check that this converges absolutely, and that $(X_t)$ is stationary and satisfies @eq-arma-ar1.
...
Formally, we say that such a solution is non-causal, unlike @eq-arma-ar1-sol.

[^3]: Because of this lack of independence, we cannot use the equations in @sec-arma-ar1-forecasts to forecast or simulate sample trajectories.

Because of the the problems of these two solutions, @eq-arma-ar1 with $|\phi| > 1$ is rarely used for modeling.

::: callout-note

One may calculate the ACVF of @eq-arma-ar1-noncausal to be
$$
...
**Theorem 10.3** (Existence and uniqueness of solutions)
Let $\phi(z)$ be the autoregressive polynomial of @eq-arma-arp.
Suppose $\phi(z) \neq 0$ for all $|z| \neq 1$, then @eq-arma-arp admits a unique stationary solution.
If futhermore $\phi(z) \neq 0$ for all $|z| \leq 1$, then the solution is causal and has the form
$$
