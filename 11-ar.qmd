...
In view of the data's periodic nature, it seems to make sense to fit a [signal plus noise model](10-fundamentals.qmd#sec-stationary-examples-signal-plus-noise) of the form
$$
...
We also make plots of its [residual diagnostics](10-fundamentals.qmd#sec-stationary-testing-white-noise).
...
First, using Proposition 11.1, the signal plus noise model is equivalent to the following:
$$
...
**Proposition 11.2** (Existence and uniqueness of solutions). 
Suppose $|\phi| < 1$, then there is a unique stationary solution to @eq-ar-ar1.
This solution is a [causal linear process](10-fundamentals.qmd#sec-stationary-linear).
...
Indeed, $(X_t)$ defined in this manner is a [causal linear process](10-fundamentals.qmd#sec-stationary-linear) and is in particular stationary.
...
**Proposition 11.3** (Mean, ACVF, ACF).
...
First, Proposition 11.3 tells us that $\phi = \rho_X(1)$, so we may use the estimator $\hat\phi \coloneqq \hat\rho_X(1)$. 
Substituting this into [Theorem 9.2](10-fundamentals.qmd#sec-stationary-moments) then gives
$$
...
$$ {#eq-ar-ar1-noncausal}
Using the same arguments as in the proof of [Proposition 11.2](10-ar.qmd#sec-ar-ar1-solutions), one can check that this converges absolutely, and that $(X_t)$ is stationary and satisfies @eq-ar-ar1.
...
**Theorem 11.4** (Existence and uniqueness of solutions)
...
In other words, @eq-ar-arp-solution is a well-defined [causal linear process](10-fundamentals.qmd#sec-stationary-linear).
...
\end{split}
$$
Finally, uniqueness can be derived in a similar manner to Proposition 11.2. $\Box$
...
- *Case 2*: If some roots are complex, then they occur in conjugate pairs[^6], and $\gamma_X(h)$ will take the form of a sum of sinusoidal curves with exponential damping.

[^6]: The conjugate of $z = a + ib$ is $\bar{z} = a - ib$.
...
**Proposition 11.6.** (Conditional distribution)
...
**Corollary 11.7** (Conditional distribution under stationarity)
...
**Proposition 11.10.** For an $\textnormal{AR}(p)$ model, both recursive and direct multi-step forecasts are the same.
...
The key observation is that random variables[^7] form an inner product space with covariance as the inner product.
...
[^7]: More accurately, the space of zero mean random variables that have finite variance and that are measurable with respect to the sigma algebra generated by the time series.
...
They are called the *Yule-Walker* equations and we reproduce them here in a more convenient form:[^8]
$$
...
[^8]: The second equation is derived by plugging in the first equation (in matrix form) into @eq-ar-moments-acvf-recursive-3.
...
$$
where $\nu_t^2 = \gamma(0) - \gamma_{1:t-1}^T\Gamma_{t-1}^{-1}\gamma_{1:t-1}$ (see Corollary 11.7) and the ACVF values $\gamma(k)$ are treated as functions in $\beta$ obtained via @eq-ar-moments-acvf-recursive-2 and @eq-ar-moments-acvf-recursive-3.
...
**Theorem 11.11.** (Large sample distribution of estimators)
...
Using Theorem 11.11, we are able to provide confidence intervals and perform hypothesis testing for model parameters.
