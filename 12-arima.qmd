...
More generally, any fully additive exponential method is equivalent to an  $\textnormal{ARIMA}(p,d,q)$ model for some choice of $p$, $d$, and $q$ (see Chapter 9.10 in @hyndman2018forecasting).
...
\left(\phi(B)(I-B)^d - I\right)\check X_{t|n} + \sum_{k=1}^q \theta_k \check W_{t-k|n} & t > n, \\
X_t & 0 < t \leq n, \\
0 & t \leq 0,
\end{cases}
...
0 & t > n \\
\phi(B)(I-B)^d \check X_{t|n} - \theta_1 \check W_{t-1|n} - \ldots \theta_q \check W_{t-q|n} & 0 < t \leq n \\
0 & t \leq 0.
\end{cases}
