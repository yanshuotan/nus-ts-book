...
```{r}
#| warning: FALSE
#| fig-cap: Kalman filtered values for log(white blood count). The observations are plotted as points and the filtered values are in blue. The blue region is a 95% confidence region.
...
```{r}
#| warning: FALSE
#| fig-cap: Kalman filtered values for log(platelet count). The observations are plotted as points and the filtered values are in blue. The blue region is a 95% confidence region.
...
```{r}
#| warning: FALSE
#| fig-cap: Kalman filtered values for hematrocrit. The observations are plotted as points and the filtered values are in blue. The blue region is a 95% confidence region.
...
Refer to Table 8.7 in @hyndman2018forecasting for the state space models corresponding to other ETS methods.
