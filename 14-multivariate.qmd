...
There are types of models, depending on whether we treat the covariates as fixed (called a fixed design), or themselves as random variables (called a random design).[^1]
...
Given two time series $(X_t)$ and $(Y_t)$, we say that $(X_t)$ Granger causes $(Y_t)$ if the dynamic regression model of $(Y_t)$ given $(X_t)$ is a better fit than a univariate AR model of $(X_t)$.
